...
def download_report(request):
    """
    Generate and download a PDF report based on the prediction details sent in the request.
    """
    try:
        data = request.data
        print("Received data:", data)  # Check if 'predicted_cost' is in the request
        ...
        prediction = float(data.get('predicted_cost', 0))  # Use 'predicted_cost' to get prediction value

        # Generate PDF report as before
        buffer = BytesIO()
        ...
        title = Paragraph("Medical Insurance Cost Prediction Report", styles['Title'])
        ...
        table_data = [
            ...
            ['Smoker', str(smoker)],
            ['Region', str(region)],
            ['Predicted Insurance Cost', f"${prediction:.2f}"]  # Ensure to format the prediction
        ]
        ...
        return HttpResponse(json.dumps({'error': 'Failed to generate report. Please try again.'}),
