  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Add code to send data to your backend for storage
        const backendResponse = await axios.post('/api/store-data', {
          formData,
          result,
        });
        if (backendResponse.status === 200) {
          ...
          console.error('Error storing data:', backendResponse.data);
        }
      } else {
