...
const InsuranceForm = () => {
  ...
  const handleDownloadReport = async () => {
    try {
      const response = await axios.post('http://127.0.0.1:8000/api/download-report/', {
        ...formData,
      ...
      // Creating a blob for the response data
      const blob = new Blob([response.data], { type: 'application/pdf' });
      const link = document.createElement('a');
      link.href = window.URL.createObjectURL(blob);
      link.download = 'insurance_prediction_report.pdf'; // Default filename
  ...
  const handleSubmit = async (event) => {
    ...
    // Check if age and BMI are positive numbers
    ...
    if (parseFloat(formData.bmi) <= 0) {
      setError('BMI must be a positive number.');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (response.status === 200) {
        setResult(response.data.prediction.toFixed(2)); // Ensure this matches the response format from the backend
      } else {
